...
def test_state_store_replaces_legacy_redirect_uri_with_configured_install_url(
        ...
        """
{
  ...
  "updated_at": "2026-03-12T00:00:00+00:00"
    ...
    monkeypatch.setenv("PUTIO_REDIRECT_URI", "http://10.10.10.179:8787/api/auth/putio/callback")
    get_settings.cache_clear()
    get_state_store.cache_clear()
    get_scheduler_service.cache_clear()
    ...
    assert (
        snapshot.settings.putio.redirect_uri
        == "http://10.10.10.179:8787/api/auth/putio/callback"
    )


def test_state_store_replaces_same_host_internal_port_redirect_with_frontend_url(
    monkeypatch, tmp_path: Path
) -> None:
    state_path = tmp_path / "state.json"
    state_path.write_text(
        """
{
  "settings": {
    "putio": {
      "app_id": "",
      "client_secret": "",
      "redirect_uri": "http://10.10.10.22:8000/api/auth/putio/callback",
      "token": null,
      "oauth_state": null,
      "account_username": null,
      "account_user_id": null,
  ...
  "updated_at": "2026-03-12T00:00:00+00:00"
    ...
    monkeypatch.setenv("GET_PUTIO_STATE_PATH", str(state_path))
    monkeypatch.setenv("FRONTEND_URL", "http://10.10.10.22:8787")
    monkeypatch.setenv("PUTIO_REDIRECT_URI", "http://10.10.10.22:8787/api/auth/putio/callback")
    get_settings.cache_clear()
    get_state_store.cache_clear()
    get_scheduler_service.cache_clear()
    ...
    assert (
        snapshot.settings.putio.redirect_uri
        == "http://10.10.10.22:8787/api/auth/putio/callback"
    )
...
def test_putio_callback_returns_to_request_origin_for_single_port_install(
    monkeypatch, tmp_path: Path
) -> None:
    monkeypatch.setenv("GET_PUTIO_STATE_PATH", str(tmp_path / "state.json"))
    monkeypatch.setenv("FRONTEND_URL", "http://10.10.10.179:8787")
    get_settings.cache_clear()
    get_state_store.cache_clear()
    get_scheduler_service.cache_clear()
    ...
    client = TestClient(app, base_url="http://10.10.10.22:8787")
    response = client.get("/api/auth/putio/callback?code=oauth-code&state=oauth-state")

    assert response.status_code == 200
    assert 'href="http://10.10.10.22:8787"' in response.text


def test_start_putio_auth_uses_request_origin_for_single_port_install(
    monkeypatch, tmp_path: Path
) -> None:
    monkeypatch.setenv("GET_PUTIO_STATE_PATH", str(tmp_path / "state.json"))
    monkeypatch.setenv("FRONTEND_URL", "http://10.10.10.179:8787")
    ...
    def seed_redirect(state) -> None:
        state.settings.putio.app_id = "8964"
        state.settings.putio.redirect_uri = "http://10.10.10.22:8000/api/auth/putio/callback"

    store.mutate(seed_redirect)

    client = TestClient(app, base_url="http://10.10.10.22:8787")
    response = client.get("/api/auth/putio/start")

    assert response.status_code == 200
    assert (
        response.json()["auth_url"]
        .find("redirect_uri=http%3A%2F%2F10.10.10.22%3A8787%2Fapi%2Fauth%2Fputio%2Fcallback")
        != -1
    )
    assert (
        get_state_store().snapshot().settings.putio.redirect_uri
        == "http://10.10.10.22:8787/api/auth/putio/callback"
    )
