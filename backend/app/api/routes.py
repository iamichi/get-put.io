...
PUTIO_CALLBACK_PATH = "/api/auth/putio/callback"
...
def is_local_request_host(hostname: str | None) -> bool:
    return hostname in {None, "localhost", "127.0.0.1", "testserver"}


def dashboard_url(request: Request, settings: Settings) -> str:
    request_port = request.url.port
    if request_port in {None, settings.port} or is_local_request_host(request.url.hostname):
        return settings.frontend_url.rstrip("/") or request_origin(request) or "/"
    return request_origin(request) or settings.frontend_url.rstrip("/") or "/"


def runtime_putio_redirect_uri(
    request: Request,
    settings: Settings,
    current_redirect_uri: str | None,
) -> str:
    request_port = request.url.port
    request_based_redirect_uri = f"{request_origin(request)}{PUTIO_CALLBACK_PATH}"
    configured_redirect_uri = (current_redirect_uri or settings.putio_redirect_uri).strip()
    if request_port == settings.port:
        return configured_redirect_uri or request_based_redirect_uri

    if not configured_redirect_uri:
        return request_based_redirect_uri

    parsed = urlparse(configured_redirect_uri)
    request_host = request.url.hostname or ""
    request_host_port = request.url.netloc

    if parsed.path != PUTIO_CALLBACK_PATH:
        return configured_redirect_uri
    if parsed.netloc in {"localhost:8000", "127.0.0.1:8000", "localhost:8787", "127.0.0.1:8787"}:
        return request_based_redirect_uri
    if parsed.hostname == request_host and parsed.netloc != request_host_port:
        return request_based_redirect_uri
    return configured_redirect_uri
...
def start_putio_auth(
    ...
    effective_redirect_uri = runtime_putio_redirect_uri(
        request,
        settings,
        state.settings.putio.redirect_uri,
    )

    if effective_redirect_uri != state.settings.putio.redirect_uri:
        def update_redirect_uri(current: AppState) -> None:
            current.settings.putio.redirect_uri = effective_redirect_uri

        store.mutate(update_redirect_uri)
        state = store.snapshot()
...
def putio_callback(
    ...
    store.mutate(mutate)
    return_url = escape(dashboard_url(request, settings), quote=True)
    return HTMLResponse(
        f"""
        <html>
